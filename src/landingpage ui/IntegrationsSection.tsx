...
import { Github, Slack, Triangle, Database, Cloud, Mail, CreditCard, LayoutDashboard, Box } from "lucide-react";

const integrations = [
  { id: 1, name: "Slack", icon: Slack, angle: 0, radius: 200, color: "text-rose-500", bg: "bg-rose-500", duration: 25 },
  { id: 2, name: "Salesforce", icon: Database, angle: 120, radius: 200, color: "text-blue-500", bg: "bg-blue-500", duration: 25 },
  { id: 3, name: "Google", icon: Mail, angle: 240, radius: 200, color: "text-emerald-500", bg: "bg-emerald-500", duration: 25 },
  
  { id: 4, name: "AWS", icon: Cloud, angle: 60, radius: 350, color: "text-amber-500", bg: "bg-amber-500", duration: 40 },
  { id: 5, name: "GitHub", icon: Github, angle: 180, radius: 350, color: "text-slate-700", bg: "bg-slate-700", duration: 40 },
  { id: 6, name: "Stripe", icon: CreditCard, angle: 300, radius: 350, color: "text-indigo-500", bg: "bg-indigo-500", duration: 40 },
  
  { id: 7, name: "HubSpot", icon: LayoutDashboard, angle: 25, radius: 500, color: "text-orange-500", bg: "bg-orange-500", duration: 60 },
  { id: 8, name: "Zapier", icon: Box, angle: 155, radius: 500, color: "text-orange-600", bg: "bg-orange-600", duration: 60 },
  { id: 9, name: "Azure", icon: Cloud, angle: 265, radius: 500, color: "text-cyan-500", bg: "bg-cyan-500", duration: 60 },
];
      ...
      <div className="max-w-[1200px] mx-auto text-center relative z-10 flex flex-col items-center">
        ...
        >
          <h2 className="text-5xl md:text-7xl font-light text-slate-900 tracking-tight mb-6">
            Connects with <span className="font-medium text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600 italic">everything.</span>
          </h2>
          <p className="text-slate-500 text-lg md:text-xl max-w-[600px] mx-auto leading-relaxed">
            BlueX integrates natively with your existing enterprise stack. Two-way sync, automated data mapping, and zero complex setup required.
          </p>
        </motion.div>
        ...
        >
            ...
            {integrations.map((integration) => {
              return (
                ...
                >
                  <div 
                    className="absolute"
                    style={{ transform: `translateY(-${integration.radius}px)`, transformStyle: 'preserve-3d' }}
                  >
                    ...
                    >
                      <div style={{ transform: 'rotateX(-65deg)' }}>
                        <div className="w-14 h-14 md:w-16 md:h-16 bg-white/90 backdrop-blur-md rounded-full border border-slate-200 shadow-xl flex items-center justify-center hover:scale-110 hover:shadow-2xl transition-all cursor-pointer relative group">
                          ...
                          <div className={`absolute inset-0 rounded-full ${integration.bg} opacity-20 blur-md group-hover:opacity-50 transition-opacity`} />
                          <integration.icon className={`w-6 h-6 md:w-7 md:h-7 ${integration.color} relative z-10`} />
          ...
          <div className="absolute z-40 flex items-center justify-center">
            <div className="w-24 h-24 md:w-32 md:h-32 bg-white rounded-full shadow-[0_0_80px_rgba(79,70,229,0.4)] flex items-center justify-center border border-indigo-100 relative group cursor-pointer hover:scale-105 transition-transform">
              ...
              <div className="absolute inset-0 rounded-full border-2 border-blue-400 animate-ping opacity-20 pointer-events-none" style={{ animationDelay: '500ms' }} />
            </div>
          </div>
