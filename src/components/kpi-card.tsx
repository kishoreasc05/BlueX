  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex items-start justify-between">
        <div className="mono-label text-muted-foreground/80 font-mono text-[9px] tracking-widest">{label}</div>
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center border border-dashed border-border/60 bg-muted/10 rounded-2xl px-6 py-20 text-center relative overflow-hidden">
      ...
      <div className="mono-label text-foreground text-xs font-semibold tracking-wider font-mono">{title}</div>
      {description ? (
        <div className="mt-3.5 max-w-sm text-xs leading-[1.65] text-muted-foreground/80">{description}</div>
      ) : null}
