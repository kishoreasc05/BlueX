...
function Dashboard() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const upcomingTasksWithProjects = await Promise.all(
        upcomingTasks.map(async (task: any) => {
          if (!task.project_id) return { ...task, project_name: "No Project" };
          const pRes: any = await supabase
            .from("projects")
            .select("name")
    ...
    <div className="space-y-6 pb-12 max-w-[1600px] mx-auto text-slate-800">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight flex items-center gap-2">
            ...
            <span className="inline-block origin-bottom-right hover:animate-pulse cursor-pointer">
              👋
            </span>
          </h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-4">
        {[
          {
            label: "Organizations",
            value: isLoading ? "-" : data?.kpi.organizations,
            icon: Building2,
          ...
          },
          {
            ...
            value: isLoading ? "-" : data?.kpi.totalProjects,
            icon: FolderKanban,
            color: "text-blue-600",
            bg: "bg-blue-50",
          ...
          },
          {
            ...
            value: isLoading ? "-" : data?.kpi.activeWorkflows,
            icon: Settings2,
          ...
          {
            label: "Revenue",
            value: isLoading ? "-" : data?.kpi.revenue,
            icon: DollarSign,
          ...
          {
            label: "Tasks Due",
            value: isLoading ? "-" : data?.kpi.tasks,
            icon: Clock,
          ...
          <div
            ...
            className="bg-white rounded-2xl p-5 border border-slate-200/60 shadow-sm flex flex-col justify-between group hover:border-indigo-300 transition-colors cursor-pointer"
          >
            <div className="flex items-start justify-between">
              <div className="text-sm font-semibold text-slate-500 line-clamp-1 group-hover:text-slate-700 transition-colors">
                {stat.label}
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* 7. Workflow Status */}
            <div
              className="bg-white rounded-2xl p-6 border border-slate-200/60 shadow-sm flex flex-col cursor-pointer hover:border-indigo-300 transition-colors"
              ...
              <div className="flex-1 grid grid-cols-2 gap-4">
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 flex flex-col justify-center">
                  ...
                  <div className="text-2xl font-bold text-emerald-600">
                    {isLoading ? "-" : data?.workflowStats.active}
                  </div>
                </div>
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 flex flex-col justify-center">
                  ...
                  <div className="text-2xl font-bold text-amber-600">
                    {isLoading ? "-" : data?.workflowStats.paused}
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-slate-50/50 border-b border-slate-100">
                  <tr>
                    <th className="px-6 py-3 font-semibold text-slate-500 text-xs uppercase tracking-wider">
                      Project
                    </th>
                    <th className="px-6 py-3 font-semibold text-slate-500 text-xs uppercase tracking-wider hidden sm:table-cell">
                      Client
                    </th>
                    <th className="px-6 py-3 font-semibold text-slate-500 text-xs uppercase tracking-wider">
                      Status
                    </th>
                    <th className="px-6 py-3 font-semibold text-slate-500 text-xs uppercase tracking-wider text-right hidden sm:table-cell">
                      Created
                    </th>
                  </tr>
                </thead>
                    ...
                    data?.recentProjects?.map((project: any, i: number) => (
                      <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={cn(
                              "text-[10px] uppercase tracking-wider font-bold px-2 py-1 rounded-md border inline-block whitespace-nowrap",
                              getStageColor(project.status),
                            )}
                          >
                    ...
                    <div key={task.id} className="flex items-start gap-3 group">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-1">
                          {task.project_name} - {new Date(task.due_date).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                  ))}
