            ...
            <DialogContent>
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input placeholder="e.g. Remote or New York, NY" value={location} onChange={(e) => setLocation(e.target.value)} />
                </div>
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button>
                <Button className="bg-indigo-600 text-white hover:bg-indigo-700" onClick={() => createJob.mutate()} disabled={!title || createJob.isPending}>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
        <div className="p-4 border-b border-slate-200 flex items-center gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <Input placeholder="Search jobs..." className="pl-9 bg-slate-50/50 border-slate-200 rounded-xl" />
          </div>
        </div>
                ...
                <tr key={job.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      ...
                      job.status === "interviewing" ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-700"
                    }`}>
                      {job.status === "open" ? "Open" : job.status === "interviewing" ? "Interviewing" : "Closed"}
                    </span>
                  </td>
