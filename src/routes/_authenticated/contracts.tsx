            ...
            <DialogContent>
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  ...
                  <Input placeholder="e.g. Website Redesign MSA" value={title} onChange={(e) => setTitle(e.target.value)} />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium">Value ($)</label>
                  <Input type="number" placeholder="e.g. 12500" value={value} onChange={(e) => setValue(e.target.value)} />
                </div>
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button>
                <Button className="bg-indigo-600 text-white hover:bg-indigo-700" onClick={() => createContract.mutate()} disabled={!title || createContract.isPending}>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
        <div className="p-4 border-b border-slate-200 flex items-center gap-4">
          <div className="relative flex-1 max-w-md">
            ...
            <Input placeholder="Search contracts..." className="pl-9 bg-slate-50/50 border-slate-200 rounded-xl" />
          </div>
        </div>
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            {isLoading ? (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-slate-500">Loading contracts...</td>
              </tr>
              ...
              contracts.map((contract: any) => (
                <tr key={contract.id} className="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4 font-medium text-slate-700">{contract.value ? `$${contract.value}` : "-"}</td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                      contract.status === "signed" ? "bg-emerald-50 text-emerald-700" : 
                      contract.status === "sent" ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-700"
                    }`}>
                      {contract.status === "draft" ? "Draft" : contract.status === "sent" ? "Sent" : "Signed"}
                    </span>
                  </td>
