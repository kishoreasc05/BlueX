...
function Page() {
  ...
  const createWorkflow = useMutation({
    mutationFn: async () => {
      const { data, error } = await (supabase as any).from("workflows").insert({
            ...
            <DialogContent>
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  ...
                  <Input placeholder="e.g. Client Onboarding" value={name} onChange={(e) => setName(e.target.value)} />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium">Trigger Condition</label>
                  <Input placeholder="e.g. New Client Added" value={trigger} onChange={(e) => setTrigger(e.target.value)} />
                </div>
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button>
                <Button className="bg-indigo-600 text-white hover:bg-indigo-700" onClick={() => createWorkflow.mutate()} disabled={!name || createWorkflow.isPending}>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {workflows.map((workflow: any) => (
            <div key={workflow.id} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm flex flex-col">
              ...
              <div className="mt-2 text-sm text-slate-500 line-clamp-2">
                <span className="font-semibold text-slate-700">Trigger:</span> {workflow.trigger_condition || "None"}
              </div>
              <div className="mt-auto pt-6 flex items-center justify-between">
                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                  workflow.status === "active" ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-700"
                }`}>
                  {workflow.status === "active" ? "Active" : "Paused"}
                </span>
                <Button variant="outline" size="sm" className="h-8 rounded-lg gap-2 text-xs font-semibold">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center shadow-sm">
          ...
          <Button className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl" onClick={() => setOpen(true)}>
